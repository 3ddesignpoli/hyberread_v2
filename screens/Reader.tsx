...
const ReaderScreen: React.FC<ReaderProps> = ({
  ...
  const togglePulseMode = () => {
    if (isPlaying) {
      ...
      setViewMode('RSVP');
      // If the user has scrolled significantly away from the bookmark, assume they want to start reading from there
      // But we should confirm if they just scrolled or actually wanted to move.
      // For now, simpler UX: Start from the View Index (where they are looking)
      // This fixes "işaretlediğim yerden başlamıyor" issue.
      if (Math.abs(viewIndex - bookmarkIndex) > 5) {
        setBookmarkIndex(viewIndex);
        indexRef.current = viewIndex;
      } else {
        setViewIndex(bookmarkIndex);
      }
      setIsPlaying(true);
